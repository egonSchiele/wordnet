 ...
 */
module.exports.lookup = async function(word, skipPointers = false) {
...
function readData(definition, skipPointers) {
  ...
  return fileHandle.read(buffer, 0, 1024, synsetOffset).then(async ({ bytesRead, buffer }) => {
...
function readIndex(filePath) {
  return new Promise((resolve) => {
    reader.read(filePath, (line) => {
      let results = parseIndexLine(line);
...
function parseIndexLine(line) {
  ...
  let [ senseCount, tagSenseCount, ...synsetOffsets] = parts;

  return synsetOffsets.map((synsetOffset) => {
    return {
    lemma,
    pos,
